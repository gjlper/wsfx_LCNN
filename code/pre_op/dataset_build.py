from code.util.excel_op import getrowls,getcolls,getexceldata
from code.util.file_fun import getlines
from code.util.str_op import getStrSegment
from code.pre_op.word2vec import vector,load_models
...
#==========================================================================
'''
将excel对应的原文书放到一个文件夹中
'''
def movews(exceldict, wsdict, targetdict):

    excelnames = os.listdir(exceldict)
    for ex in excelnames:
        wsname = ex.strip().split('_')[1]
        spath = os.path.join(wsdict, wsname)

        try:
            print(spath)
            shutil.copy(spath,targetdict)
            print('Move:'+wsname)
        except:
            print('ERROR'+ex)

exceldict = '../../source/训练集'
wsdict = '/Users/wenny/nju/task/法条文书分析/2014filled/法条内容填充2014/2014'
targetdict = '../../source/训练集原文书'


movews(exceldict,wsdict,targetdict)
