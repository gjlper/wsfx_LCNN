...
from code.models.mlp_polar import mlpmodel
from code.models.config import Config
from code.train.loader import batch_iter,batch_iter_test,data_load
...
save_dir  = '../../result/set4/mlp_polar'  #修改处
ckpath = 'times:1'
tbpath = 'times:1'
...
def feed_data(x1_batch,x2_batch,polar_batch,y_batch, keep_prob):
    feed_dict = {
        model.input_x1: x1_batch,
        model.input_x2: x2_batch,
        model.input_x_polar: polar_batch,
...
def evaluate(sess, x1_,x2_,polar_,y_):
    """评估在某一数据上的准确率和损失"""
    data_len = len(x1_)
    batch_eval = batch_iter(x1_, x2_,polar_,y_, 128)
    total_loss = 0.0
    total_acc = 0.0
    for x1_batch,x2_batch, polar_batch, y_batch in batch_eval:
        batch_len = len(x1_batch)
        feed_dict = feed_data(x1_batch,x2_batch,polar_batch, y_batch, 1.0)
...
def train():
    ...
    train_1,train_2,train_ks, train_output = data_load(t_f,config)
    #use n-gram**************
    ...
    # print(train_3)
    val_1, val_2,val_ks, val_output = data_load(v_f,config)
    # use n-gram**************
...
def test():
    ...
    start_time = time.time()
    x1_test, x2_test,ks_test, y_test = data_load(test_f, config)
    #n-gram
    ...
    for i in range(num_batch):  # 逐批次处理
        ...
        feed_dict = {
            ...
            model.input_x_polar: ks_test[start_id:end_id],
